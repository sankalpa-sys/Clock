...
function Weather() {
  ...
  return (
    <main className="relative h-screen select-none w-full bg-black flex items-center justify-center ">
      <Head>
        <title>Mausam</title>
      ...
      <Fade left>
      {!error ? (
        <section className={` h-auto py-10 w-[90%] lg:w-[30%] shadow-xl shadow-gray-100/40 bg-gray-100 rounded-lg items-center space-y-5 flex-col ${!weather || !sys || !main ? "hidden": "flex"}`}>
        ...
        <h1 className="font-bold text-gray-100 text-4xl font-mono animate-bounce">CITY NOT FOUND!</h1>
      )}
        </Fade>
      ...
      <Link href={"/"}>
        <p className="cursor-pointer text-red-600 border-b px-1 border-red-600 absolute top-3 right-3">
          Time
        </p>
      </Link>

    <Fade up>
    <div className={` flex items-center border shadow-xl shadow-black-30 px-2 rounded-full ${position !== "absolute"?"  lg:w-[50%] w-[80%]":"absolute w-[70%] lg:w-[25%] top-3 left-3"}`}>
      ...
      <SearchIcon onClick={handleClick} className="h-6 w-6 rounded-full  text-gray-100 cursor-pointer"/>
    </div>
    </Fade>
